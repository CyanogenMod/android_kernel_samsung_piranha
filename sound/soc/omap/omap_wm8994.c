...
#define HOLD_CAP_CONSTRAINTS_WA
...
#ifdef HOLD_CAP_CONSTRAINTS_WA
static struct pm_qos_request_list capture_pm_qos_handle;
#define SET_CAPTURE_PM_QOS_DELAY 400
#define CLEAR_CAPTURE_PM_QOS_DELAY -1
#endif //HOLD_CAP_CONSTRAINTS_WA
...
#ifdef HOLD_CAP_CONSTRAINTS_WA
static int omap4_hifi_start_up(struct snd_pcm_substream *substream,
                              struct snd_pcm_hw_params *params)
{
	...
	/* Holding the C3 and C4 contraints when capture stream starts */
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE){
		pm_qos_update_request(&capture_pm_qos_handle,SET_CAPTURE_PM_QOS_DELAY);
		printk("\nC3 and C4 constraints are held before capture starts..\n");
	}
	
        return 0;
}

static int omap4_hifi_shutdown(struct snd_pcm_substream *substream,
                              struct snd_pcm_hw_params *params)
{
        struct snd_soc_pcm_runtime *rtd = substream->private_data;
        struct snd_soc_dai *codec_dai = rtd->codec_dai;
	...
	/* Releasing the contraints when capture stream is stopped */
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE){
		pm_qos_update_request(&capture_pm_qos_handle,CLEAR_CAPTURE_PM_QOS_DELAY);
		printk("\nC3 and C4 constraints are released after capture stopped..\n");
	}
...
#ifdef HOLD_CAP_CONSTRAINTS_WA
	.startup = omap4_hifi_start_up,
	.shutdown = omap4_hifi_shutdown,
#endif // HOLD_CAP_CONSTRAINTS_WA 
...
#ifdef HOLD_CAP_CONSTRAINTS_WA
        pm_qos_add_request(&capture_pm_qos_handle, PM_QOS_CPU_DMA_LATENCY,
                                                PM_QOS_DEFAULT_VALUE);
#endif //HOLD_CAP_CONSTRAINTS_WA
...
#ifdef HOLD_CAP_CONSTRAINTS_WA
	pm_qos_remove_request(&capture_pm_qos_handle);
#endif //HOLD_CAP_CONSTRAINTS_WA
