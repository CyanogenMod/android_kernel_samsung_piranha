...
static void battery_manager_work(struct work_struct *work)
{
	...
	if (first_reset == 0 && di->bat_info.vcell >= 3500000
		&& di->pdata->bootmode == 5) {
		first_reset = 1;
		fuel_reset = 1;
		di->pdata->set_charger_en(0);
		msleep(200);
		GasGauge_Reset();
		pr_info("%s: GasGauge_Reset\n", __func__);
		msleep(300);
		di->pdata->set_charger_en(1);
		}

	if (di->bat_info.vcell >= 3500000 &&
		(prev_vcell < 3500000 && prev_vcell != 0)
		&& first_reset == 0 && di->pdata->bootmode == 5) {
		first_reset = 1;
		fuel_reset = 1;
		di->pdata->set_charger_en(0);
		msleep(200);
...
static int __devinit batman_probe(struct platform_device *pdev)
{
	...
	if (charger_in) {
		...
		di->charge_status = POWER_SUPPLY_STATUS_CHARGING;
		di->is_cable_attached = true;
	} else if (di->pdata->get_charger_type()) {
		pr_info("%s: power on checked TA FULL\n", __func__);
		di->charge_status = POWER_SUPPLY_STATUS_FULL;
		di->is_cable_attached = true;
		di->once_fully_charged = true;
		di->discharge_status = STATUS_BATT_REAL_FULL;
